...
public class TallerService : ITallerService
{
    ...
    public async Task<PaginationResponse<InscritosPorTallerDtoResponse>> ListAsync(BusquedaInscritosPorTallerRequest request)
    {
        ...
        try
        {
            var tuple = await _repository.ListAsync(request.InstructorId,
                request.Taller, request.Situacion, request.FechaInicio, request.FechaFin, request.Pagina,
                request.Filas);

            response.Data = _mapper.Map<ICollection<InscritosPorTallerDtoResponse>>(tuple.Collection);
            response.TotalPages = Helper.GetTotalPages(tuple.Total, request.Filas);
